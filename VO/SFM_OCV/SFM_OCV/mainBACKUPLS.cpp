...
Mat<float> initializeInvDM(Frame<float>* frame,float gradIntTreshold = 255/10);
...
float exhaustiveSearchEpipolarLine( Frame<float>* f1, Frame<float>* f2, Mat<float>* invDepthMap1, Mat<float>* x1, Mat<float>* x2, Mat<float>* R, Mat<float>* t, float var_invDepthMap, Mat<float>* K, Mat<float>* invK, float factor = 1.0/20);
...
Mat<float> poseEstimationLSGRAD(Frame<float>* f1, Mat<float>* dm1, Frame<float>* f2, Frame<float>* gradI1X, Frame<float>* gradI1Y, Mat<float>* K, Mat<float>* invK, Mat<float>* var_invDepthMap1, Mat<float>* RI);
template<typename T>
...
Mat<float> initializeInvDM(Frame<float>* frame, float gradIntTreshold)
{
    ...
    for(int i=frame->getLine();i--;)
    {
        ...
        for(int j=frame->getColumn();j--;)
        {
            ...
            {
                idm.set( (float)0.9+(float)gen.dev(), i+1,j+1);
...
inline cv::Mat computeGradient(cv::Mat* im)
{
    ...
    //cv::GaussianBlur(*im,*im,cv::Size(0,0),(float)1.0/sqrt(2),(float)1.0/sqrt(2));
...
inline float exhaustiveSearchEpipolarLine( Frame<float>* f1, Frame<float>* f2, Mat<float>* invDepthMap1, Mat<float>* x1, Mat<float>* x2, Mat<float>* R, Mat<float>* t, float var_invDepthMap, Mat<float>* K, Mat<float>* invK, float factor)
{
    float lambda;
    Mat<float> Xlambda(3,1);
    Mat<float> x_2(3,1);
    int nbrHyp = 10;
    float varStep = ((float)4.0/nbrHyp)*var_invDepthMap;
    ...
    {
        lambda = (float)1.0/(invDepthMap1->get(x1->get(1,1),x1->get(2,1)) - 4.0*var_invDepthMap + i*varStep );
...
int main10()
{
    float gradIntTreshold = 100;
    clock_t timertotal = clock();
    clock_t timerdepth = clock();
    float factor = (float)1.0/8;
    int count = 0;
    float fps = (float)2.0;
    float goalfps = (float)2.0;
    float coeff = (float)2.0;
    ...
    int nbrKF = 0;
    bool kframeChange = false;
    bool withMultipleKF = false;
    int nbrFramePerKF = ceil(1.0/ceil(fps))*4;
    ...
    Mat<float> line2;
    Mat<float> line22;
    float numlim = (float)10000000.0/numeric_limits<float>::epsilon();
    ...
    K.set((float)830,1,1); //fx
    K.set((float)830,2,2); //fy
    ...
    {        
        ...
        f2 = Frame<float>( cv2Matp<float>( computeGradient(&frame2) ) );
                ...
                if(invDepthMap1.get(i+1,j+1) != 0.0 )
                {
                    ...
                    */
                    ...
                    if(f1.get(x1).get(1,1) >= (float)gradIntTreshold)
                    {
                        ...
                        cv::line(debugIm,
                                 cv::Point(((float)1.0/factor)*x2.get(1,1)-100000*line2.get(1,1), ((float)1.0/factor)*x2.get(2,1)-100000*line2.get(2,1)),
                                 cv::Point(((float)1.0/factor)*x2.get(1,1)+ 100000*line2.get(1,1), ((float)1.0/factor)*x2.get(1,1)+100000*line2.get(2,1)),
                                 colorbleue);
                        */
                        ...
                        cv::line(debugIm,
                                 cv::Point(((float)1.0/factor)*x2.get(1,1)-numlim*line22.get(1,1), ((float)1.0/factor)*x2.get(2,1)-numlim*line22.get(2,1)),
                                 cv::Point(((float)1.0/factor)*x2.get(1,1)+ numlim*line22.get(1,1), ((float)1.0/factor)*x2.get(1,1)+numlim*line22.get(2,1)),
                                 colorvert);
                        ...
                        */
                        ...
                        float estimatedDepth = (float)1.0/(exhaustiveSearchEpipolarLine( &f1,
                                                                                   &f2,
                                                                                   &invDepthMap1,
                                                                                   &x1,
                                                                                   &x2,
	...
	{
		/*
		invDepthMap1 = invDepthMap1 % invDepthMap1 % invDepthMap1;
		for(int i=invDepthMap1.getLine();i--;)
		{
			for(int j=invDepthMap1.getColumn();j--;)
			{
				
				invDepthMap1.set( (invDepthMap1.get(i+1,j+1) != (float)0 ? 1.0/invDepthMap1.get(i+1,j+1) : (float)0 ), i+1, j+1);
			}
		}
		*/
        ...
        {		
        	...
        	nbrKF++;
        	f1 = Frame<float>( cv2Matp<float>( computeGradient(&newKF) ) );
        	invDepthMap1 = initializeInvDM(&f1, gradIntTreshold) + propagateDepthMap( &K, &invK, &deltaPose,&invDepthMap1);
...
Mat<float> poseEstimationLSGRAD(Frame<float>* f1, Mat<float>* dm1, Frame<float>* f2, Frame<float>* gradI1X, Frame<float>* gradI1Y, Mat<float>* K, Mat<float>* invK, Mat<float>* var_invDepthMap1, Mat<float>* RI)
{
    ...
    LSSE3<float> instanceLS(f1,f2, gradI1X, gradI1Y, K,invK,dm1, var_invDepthMap1, nbrL);
    Mat<float> X( instanceLS.energy( Mat<float>((float)nbrL,1,1) ) );
    cout << " L'execution de LSSE3 a prise : " << (float)(clock()-timer)/CLOCKS_PER_SEC << " secondes." << endl;
    X.afficher();
    *RI = instanceLS.getResidual(nbrL);
