...
class ONSCNewton
{
	 ...
	 **/
    ONSCNewton(  OO<T>* ooInst, int it = 1, bool approxHessian = false)
	{
//		int counter = 0;	//count le nombre d'essai de rattraper une mauvaise variation..
	...
	Mat<T> computeGrad()
	{
		...
		Mat<T> delta((T)0, n,1);
		delta.set(h, 1,1);
		Mat<T> grad( instance->energy(x_k[k]+delta) - instance->energy(x_k[k]-delta)    );        
		
		for(int i=2;i<=n;i++)
		{
			delta.set((T)0,i-1,1);
			delta.set(h,i,1);
			grad = operatorC(grad, instance->energy(x_k[k]+delta) - instance->energy(x_k[k]-delta)    );		
		}        
    ...
    inline Mat<T> computeHessian()
	{
		...
		for(int j=1; j<=n;j++)
		{
			Mat<T> hess1( instance->energy(x_k[k]+delta2+delta1) - instance->energy(x_k[k]+delta2-delta1)  );
			Mat<T> hess2( instance->energy(x_k[k]-delta2+delta1) - instance->energy(x_k[k]-delta2-delta1) );			
			for(int i=2;i<=n;i++)
			{
				delta1.set((T)0,i-1,1);
				delta1.set(h,i,1);
				
				hess1 = operatorC(hess1,  instance->energy(x_k[k]+delta2+delta1) - instance->energy(x_k[k]+delta2-delta1));		
				hess2 = operatorC(hess2,  instance->energy(x_k[k]-delta2+delta1) - instance->energy(x_k[k]-delta2-delta1));		
			}
