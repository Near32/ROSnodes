...
template<typename T>	/*pas de point virgule en fin de ligne...*/
inline Mat<T> operatorL(const Mat<T>& a, const Mat<T>& b);
template<typename T>	/*pas de point virgule en fin de ligne...*/
Mat<T> Line( const Mat<T>& a, int ind);
template<typename T>	/*pas de point virgule en fin de ligne...*/
inline Mat<T> operatorC(const Mat<T>& a, const Mat<T>& b);
template<typename T>	/*pas de point virgule en fin de ligne...*/
...
template<typename T>	/*pas de point virgule en fin de ligne...*/
inline Mat<T> sum(const Mat<T>& a);
template<typename T>	/*pas de point virgule en fin de ligne...*/
...
Mat<T> operatorL(const Mat<T>& a, const Mat<T>& b)
{
    ...
    if(al == bl)
    {
        ...
        Mat<T> r(a, (T)0, (int)1, (int)1, al, rc);

        for(int i=1;i<=al;i++)
        {
            for(int j=ac+1;j<=rc;j++)
            {
                r.set( b.get(i,j-ac),i,j);
            }
        }
...
Mat<T> inv(const Mat<T>& a)
{
    Mat<T> temp( (a.getLine()==a.getColumn()? a :transpose(a)*a));
    ...
    return (a.getColumn()==a.getLine()? temp : temp*transpose(a));
}
...
Mat<T> extract(Mat<T> m, int ib, int jb, int ie, int je)
{
    ...
    if( (ie-ib+1) >= 1 && (je-jb+1) >= 1)
    {
        r = Mat<T>(ie-ib+1, je-jb+1);

        for(int i=1;i<=r.getLine();i++)
        {
            for(int j=1;j<=r.getColumn();j++)
            {
                r.set( (T)(m.get(ib+i-1,jb+j-1)), i, j);
            }
        }
    }
    else
           cerr << "EXTRACTION IMPOSSIBLE..." << endl;
...
template<typename T>	/*pas de point virgule en fin de ligne...*/
inline Mat<T> correlation(Mat<T> m, Mat<T> k)
{    
    Mat<T> r(m);    
    int mi = m.getLine();
    int mj = m.getColumn();
    ...
    if(mj >= kj && mi >= ki)
    {
        for(int i=1;i<=mi;i++)
        {
            for(int j=1;j<=mj;j++)
            {                
                //T temp = (T)(sum(sum( k % extract(m, i-ki/2, j-kj/2, i+ki/2, j+kj/2))).get(1,1));
                // inlining....!
                ...
                for(int ii=i-ki/2;ii<=i+ki/2;ii++)
                    for(int jj=j-kj/2;jj<=j+kj/2;jj++)
                        ...
                        //temp += k.mat[ii-i+ki/2+1-1][jj-j+kj/2+1-1]*m.mat[ii-1][jj-1];
	...
	for(int i=1;i<=nr;i++)
	{
		for(int j=1;j<=mr;j++)
		{
            ...
            for(int ii=1;ii<=nk;ii++)
            {
                ...
                {
                    temp += m.get((i-nk/2)+ii-1, (j-mk/2)+jj-1)*k.get(ii,jj);
                }
            }

            //inlining !!!!
            //T temp = (T)( sum( sum( extract(m, (i-1)*nk+1, (j-1)*mk+1,  i*nk, j*mk) % k))).get(1,1);
...
inline int cv2Matp( cv::Mat mat, Mat<T>* red, Mat<T>* green, Mat<T>* blue)
{
    ...
    *blue = Mat<T>((T)0, r,c);
    ...
    for(int i=r;i--;)
    {
        ...
        {
            red->set( (T)p[j], i+1, (int)((double)(j)/channels)+1);
            //red->mat[i][(int)((double)j/channels)] = (T)p[j];
            //red->mat[i*(int)((double)j/channels)] = (T)p[j];
            green->set( (T)p[j+1], i+1, (int)((double)(j)/channels)+1);
            //green->mat[i][(int)((double)j/channels)] = (T)p[j+1];
            //green->mat[i*(int)((double)j/channels)] = (T)p[j+1];
            blue->set( (T)p[j+2], i+1, (int)((double)(j)/channels)+1);
            //blue->mat[i][(int)((double)j/channels)] = (T)p[j+2];
            //blue->mat[i*(int)((double)j/channels)] = (T)p[j+2];
        }
...
template<typename T>	//pas de point virgule en fin de ligne...
inline Mat<T> cv2Matp( cv::Mat mat)
{
    if( mat.channels() != 1)
...
template<typename T>	/*pas de point virgule en fin de ligne...*/
Mat<T> hhTrans(const Mat<T>& x);
template<typename T>	/*pas de point virgule en fin de ligne...*/
Mat<T> hhTransMat(const Mat<T>& A, int k);
template<typename T>	/*pas de point virgule en fin de ligne...*/
    ...
    QR(Mat<T> A, Mat<T>* Q, Mat<T>* R)
    {
        ...
        for(int i=1;i<=nbrM-1;i++)
        {
            ...
            *R = (*M[i-1])*(*R);
            ...
            Mat<T> invM(tranpose(*M[i-1]) );//M[i-1]->getInv());
            *Q = (*Q)*invM;
        }
