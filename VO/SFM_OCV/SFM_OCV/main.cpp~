...
int main10()
{
    ...
    if(grad)
    	invDepthMap1 =  initializeInvDM(&f1Grad, gradIntTreshold);
    else
	...
	else
	{
        ...
        deltaPose = poseEstimationLSGRAD(&f1, &invDepthMap1,&f2, &f2GradX, &f2GradY, &K, &invK, &var_invDepthMap1, &ResidualImage);
	...
	if(debug)
	{
        ...
        {		
        	...
        	f1 = f2;
        	invDepthMap1 = initializeInvDM(&f2Grad, gradIntTreshold) + propagateDepthMap( &K, &invK, &deltaPose,&invDepthMap1);
...
Mat<float> poseEstimationLSGRAD(Frame<float>* f1, Mat<float>* dm1, Frame<float>* f2, Frame<float>* gradI2X, Frame<float>* gradI2Y, Mat<float>* K, Mat<float>* invK, Mat<float>* var_invDepthMap1, Mat<float>* RI)
{
    clock_t timer = clock();
    int nbrL = 3;
